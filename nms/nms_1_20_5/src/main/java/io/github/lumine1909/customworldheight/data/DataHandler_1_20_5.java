...
public class DataHandler_1_20_5 implements DataHandler<DimensionType, Holder<DimensionType>, ResourceKey<DimensionType>> {
    ...
    public LevelData<DimensionType, ResourceKey<DimensionType>, Holder<DimensionType>> createData(Identifier id, Height height, BaseDimensionType dimension) {
        LevelData<DimensionType, ResourceKey<DimensionType>, Holder<DimensionType>> levelData = new LevelData_1_20_5(id, height, dimension);
        switch (dimension) {
    ...
    public void processData(LevelData<DimensionType, ResourceKey<DimensionType>, Holder<DimensionType>> data, Holder<DimensionType> holder) {
        ...
        ResourceKey<DimensionType> newResourceKey = ResourceKey.create(Registries.DIMENSION_TYPE, new ResourceLocation(data.getId().namespace(), data.getId().value()));
    ...
    public void processWorld(World world, LevelData<DimensionType, ResourceKey<DimensionType>, Holder<DimensionType>> data) {
        ...
        EntityLookupProxy entityLookup = EntityLookupProxy.wrap(serverLevel.getEntityLookup());
        entityLookup.setMinSectionY(minSectionY);
        entityLookup.setMaxSectionY(maxSectionY);

        StarLightInterfaceProxy lightInterface = StarLightInterfaceProxy.wrap(serverLevel.getChunkSource().getLightEngine().theLightEngine);
